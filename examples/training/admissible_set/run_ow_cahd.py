...
def main():
    cfg = load_config("admissible_ow_cahd.yaml")
    llm_cfg = cfg["llm"]
    ...
    for round_id, wake_batch in enumerate(wake_stream):
        ...
        if result.eohs_samples_used <= 0:
            raise RuntimeError("ADMISSIBLE OW-CAHD made no EOHS sample progress.")
    ...
    print(f"ADMISSIBLE OW-CAHD logs written to {logger.log_dir}")
