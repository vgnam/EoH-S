...
def main():
    cfg = load_config("bp1d_ow_cahd.yaml")
    llm_cfg = cfg["llm"]
    ...
    for round_id, wake_batch in enumerate(wake_stream):
        ...
        if result.eohs_samples_used <= 0:
            raise RuntimeError("BP1D OW-CAHD made no EOHS sample progress.")
    ...
    print(f"BP1D OW-CAHD logs written to {logger.log_dir}")
