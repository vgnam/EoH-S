...
def load_config():
    return yaml.safe_load((REPO_ROOT / "cfg" / "cvrp_eoh.yaml").read_text(encoding="utf-8"))
...
def main():
    ...
    profiler = EoHProfiler(**profiler_cfg)
    method = EoH(llm=llm, profiler=profiler, evaluation=task, **cfg["method"])
    method.run()
    ...
    print(f"Final EoH population saved to {final_population_path}")
    for hidden_dataset_path in hidden_dataset_paths(hidden_test_cfg):
        try:
            ...
            rows, output_path = save_hidden_utility_post_eval(
                ...
                portfolios_by_round,
                hidden_dataset_path,
                portfolio_protocol="fixed final EoH population",
                round_workers=1,
            ...
            print_hidden_utility_post_eval("eoh", rows, output_path)
