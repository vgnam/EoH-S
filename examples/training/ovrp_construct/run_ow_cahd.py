...
def main():
    cfg = load_config("ovrp_ow_cahd.yaml")
    llm_cfg = cfg["llm"]
    ...
    for round_id, wake_batch in enumerate(wake_stream):
        ...
        if result.eohs_samples_used <= 0:
            raise RuntimeError("OVRP OW-CAHD made no EOHS sample progress.")
    ...
    print(f"OVRP OW-CAHD logs written to {logger.log_dir}")
