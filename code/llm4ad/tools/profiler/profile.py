...
class ProfilerBase:
    ...
    def _write_json(self, function: Function, *, record_type: Literal['history', 'best'] = 'history', record_sep=200):
        ...
        content = {
            ...
            'score_list': function.score_list
        }
    ...
    def _record_and_print_verbose(self, function, *, resume_mode=False):
        ...
        if self._num_objs < 2:
            if type(score) is not list and score is not None and score > self._cur_best_program_score:
                ...
                if score is None:
                    ...
                    if self._num_objs < 2:
                        if type(score) is list:
                            print(
                                f'Sample{self.__class__._num_samples}: Average Score={np.mean(score)}  ')
                        else:
