...
class BP2DEvaluation(Evaluation):
    ...
    def evaluate_2d(self, eva: Callable) -> float:
        ...
        for instance in self._datasets[:self.n_instance]:
            ...
            result = self.pack_items_2d(item_dimensions, bin_dimensions, eva, self.n_bins)
            if result is None:
                return None
            num_bins, _ = result
            scores.append(-float(num_bins))

        if not scores:
            return None
        if self.return_list:
            return scores
        return sum(scores) / len(scores)  # Negative because we want to minimize bins
