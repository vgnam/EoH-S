...
class CFLPEvaluation(Evaluation):
    ...
    def assign_customers(self, facility_capacities: List[int], customer_demands: List[int], assignment_costs: List[List[int]], eva: Callable) -> Tuple[int, List[List[int]]]:
        ...
        def infeasibility_penalty():
            max_cost = max(max(row) for row in assignment_costs)
            return total_cost + (len(remaining_customers) + 1) * max_cost * 10
        ...
        while remaining_customers:
            ...
            if selected_customer not in remaining_customers:
                return infeasibility_penalty(), assignments
            if not isinstance(selected_facility, (int, np.integer)) or not 0 <= int(selected_facility) < n_facilities:
                return infeasibility_penalty(), assignments
